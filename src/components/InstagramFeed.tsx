...
const FALLBACK_IMAGES = [
  {
    src: "https://images.unsplash.com/photo-1455659814943-ff9fe1c875eb?w=400&q=80",
    alt: "Wild garden flowers",
  },
  {
    src: "https://images.unsplash.com/photo-1462277899769-7e7d2a7f2f2f?w=400&q=80",
  ...
  {
    src: "https://images.unsplash.com/photo-1470058869952-7a8d7daf08e1?w=400&q=80",
...
export async function InstagramFeed({ handle = "front_porchflowers" }: InstagramFeedProps) {
  const profileUrl = getInstagramProfileUrl(handle);
  const posts = await getInstagramPosts(8);
  const connected = isInstagramConfigured() && posts.length > 0;
          ...
          {connected
            ? posts.slice(0, 8).map((post) => (
                ...
                >
                  <Image
                    src={post.mediaUrl}
                    alt={post.caption?.slice(0, 100) || "Front Porch Flowers on Instagram"}
                    fill
                  ...
                  <div className="absolute inset-0 bg-charcoal/0 group-hover:bg-charcoal/30 transition-colors flex items-center justify-center">
                    <svg
                      className="w-8 h-8 text-cream opacity-0 group-hover:opacity-100 transition-opacity"
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      aria-hidden="true"
                    >
